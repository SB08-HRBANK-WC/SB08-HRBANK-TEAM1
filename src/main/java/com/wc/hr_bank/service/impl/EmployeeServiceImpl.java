...

{
...

    {
        ...
        if (profileImage != null && !profileImage.isEmpty()) {
            profileFile = saveProfileImage(profileImage);
        }
    ...
    private File saveProfileImage(MultipartFile multipart) {
        String contentType = multipart.getContentType();
        if (contentType == null || !ALLOWED_IMAGE_TYPES.contains(contentType)) {
            throw new IllegalArgumentException("지원하는 프로필 이미지 형식이 아닙니다: jpeg, jpg, png, webp");
        }
        String ext = switch (contentType) {
            ...
            default -> throw new IllegalArgumentException("지원하지 않는 이미지 형식: " + contentType);
        };

        File file = fileRepository.save(new File("temp", contentType, 0L, null));
        try (OutputStream out = fileStorage.save(file.getId(), ext);
        ...
        String fileName = (originalName != null && !originalName.isBlank())
                ? originalName
                : ("profile_" + file.getId() + ext);
        long size = multipart.getSize();
        file.update(fileName, contentType, size, null);
...

    {
        ...
        File profileImageToUse = employee.getProfileImage();
        if (profileImage != null && !profileImage.isEmpty()) {
            profileImageToUse = saveProfileImage(profileImage);
        }
...

    {
        ...
        if (employee.getStatus() == EmployeeStatus.RESIGNED) {
            throw new IllegalArgumentException("퇴사 상태인 직원은 삭제할 수 없습니다.");
        }
    ...
    public List<EmployeeDistDto> getEmployeesDist(String groupBy, EmployeeStatus status) {
        List<Object[]> dist= null;

        if(groupBy.equals("department")) {
            // 부서별
            dist = employeeRepository.getEmployeesDistByDepartment(status);
        }else {
            // 직무별
            dist = employeeRepository.getEmployeesDistByPosition(status);
        }
